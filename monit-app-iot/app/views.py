...
def getHost(id, category):
	...
	if category=='1':
		rows = models.Resource.query.filter_by(host=host).with_entities(models.Resource.host, models.Resource.date,
																		models.Resource.time, models.Resource.cpu).all()
		title = "CPU Usage"
	elif category=='2':
		rows = models.Resource.query.filter_by(host=host).with_entities(models.Resource.host, models.Resource.date,
																		models.Resource.time, models.Resource.memory_avail).all()
		title = "Memory Available"
	elif category=='3':
		rows = models.Resource.query.filter_by(host=host).with_entities(models.Resource.host, models.Resource.date,
																		models.Resource.time, models.Resource.memory_used).all()
		title = "Memory Used"
	elif category=='4':
		rows = models.Resource.query.filter_by(host=host).with_entities(models.Resource.host, models.Resource.date,
																		models.Resource.time, models.Resource.swap_free).all()
		title = "Swap Free"
	return render_template('res.html', rows=rows, host_list=host_list, host_id=id, category=category, length=len(rows), title=title)
@app.route('/net' )
def getnet():
	host = models.Host.query.first()
...
def getAllNet(id, category):
	...
	host = models.Host.query.filter_by(id=id).options(load_only('host'))
	if category == '1':
		rows = models.Network.query.filter_by(host=host).with_entities(models.Network.host, models.Network.byte_receive,
																		models.Network.byte_sent).all()
		title = "Byte Traffic"
		cat = ("Byte Sent", "Byte Receive")
	elif category == '2':
		rows = models.Network.query.filter_by(host=host).with_entities(models.Network.host, models.Network.packet_receive,
																		models.Network.packet_sent).all()
		title = "Packet Traffic"
		cat = ("Packet Sent", "Packet Receive")
	return render_template('net.html', rows=rows, host_list=host_list, host_id=id, category=category, length=len(rows),
						   title=title, cat=cat)
@app.route('/available' )
def getstatus():
