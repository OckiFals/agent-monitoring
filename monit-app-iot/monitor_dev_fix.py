...
class ServerProtocol(DatagramProtocol):
    ...
    def datagramReceived(self, data, (host, port)):
        ...
        if result[0]==1:
            ...
            VALUES (?,?,?,?,?,?,?);''',(result[1],str(time.strftime("%d-%m-%Y")),str(time.strftime("%H:%M:%S")),result[2],result[3],result[4],result[5]))
            ...
            cursor.execute('''INSERT INTO network(host,date,time,byte_sent,byte_receive,packet_sent,packet_receive)
                            VALUES (?,?,?,?,?,?,?);''', (
                result[1], str(time.strftime("%d-%m-%Y")),str(time.strftime("%H:%M:%S")), result[2], result[3], result[4], result[5]))
            print "network has been inserted to database"
            print "result %r " % (result)
        elif result[0]==3:
            cursor.execute('''INSERT INTO availability(host,date,time,status)
                                            VALUES (?,?,?,?);''', (result[1],str(time.strftime("%d-%m-%Y")),str(time.strftime("%H:%M:%S")), result[3]))
            ...
            if count == 0:
                cursor.execute('''INSERT INTO host(hostname,host,status,phase) VALUES (?,?,?,?);''',(result[0], result[1],result[2],result[3]))
                print "insert"
            else:
...
def main():
    reactor.listenUDP(9964, ServerProtocol('server', '', 9964))
    reactor.run()
